...
class LoginForm(FlaskForm):
    username = StringField('用户名', validators=[Required(), Length(3, 24, message='用户名长度在3-24字符之间')])
    password = PasswordField('密码', validators=[Required(), Length(6, 24, message='密码长度在6-24字符之间')])
...
class CourseForm(FlaskForm):
    name = StringField('课程名称', validators=[Required(), Length(5, 23)])
...
class UserForm(FlaskForm):
    ...
    repeat_password = PasswordField('确认密码', validators=[Required(), Length(6, 24),
        EqualTo('password')])
    role = IntegerField('权限', validators=[Required(), NumberRange(min=10, max=30)])
...
class LiveForm(FlaskForm):
    ...
    user_id = IntegerField('直播用户ID', validators=[Required()]) 
    submit = SubmitField('提交')
